...
def get_models(model_urls: dict):
    ...
    """
    # Download and load DECIMER-V2 model (required)
    model_paths = {}
    model_paths.update(
        utils.ensure_models(default_path=default_path, model_urls={"DECIMER": model_urls["DECIMER"]})
    )

    # Try to download/load HandDrawn model (optional)
    handdrawn_available = False
    try:
        model_paths.update(
            utils.ensure_models(
                default_path=default_path,
                model_urls={"DECIMER_HandDrawn": model_urls["DECIMER_HandDrawn"]},
            )
        )
        handdrawn_available = True
    except Exception as e:
        # NOTE: In some networks, zenodo 'records' endpoint can be blocked.
        # We allow running with the standard DECIMER-V2 model.
        print(f"⚠️ HandDrawn model unavailable (will use standard model only): {e}")

    # Load tokenizers (from DECIMER model directory)
    ...
    DECIMER_Hand_drawn = None
    if handdrawn_available and "DECIMER_HandDrawn" in model_paths:
        try:
            DECIMER_Hand_drawn = tf.saved_model.load(model_paths["DECIMER_HandDrawn"])
        except Exception as e:
            DECIMER_Hand_drawn = None
            print(f"⚠️ Failed to load HandDrawn model (will use standard model only): {e}")
