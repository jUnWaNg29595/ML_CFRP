...
熔点筛选使用独立的回归模型，不会复用 Tg 模型、普通性能模型或旧的经验熔点公式。推荐按以下顺序操作：

1. 在“虚拟分子筛选”页面打开“熔点数据集采集”，分别配置树脂和固化剂的 SMARTS、CID 上限及固化剂类别；树脂和固化剂上限彼此独立。
2. 运行 PubChem 采集后，同时保留原始记录、解析后的清洗数据、数据摘要和缓存进度。清洗数据默认只将有限的 `mp_c` 且 `mp_quality=high` 的记录交接给模型训练；低质量记录需要手动开启。
3. 点击“载入模型训练”，将交接数据的目标列设为 `mp_c`，完成分子特征工程和回归训练。训练 artifact 会记录 `task_kind=melting_point`、摄氏单位、数据指纹、角色计数、质量策略和特征工作流哈希。
4. 在筛选页上传独立熔点模型。模型校验失败时不会覆盖当前普通模型或 Tg 模型，也不会静默使用其他模型代替。
5. 熔点门控分为两种模式：
   - `annotate`（默认）：保留全部候选，并增加预测值、标准差、适用域分数、筛选状态和原因，适合先观察分布。
   - `strict`：仅保留每个已启用角色均通过的候选；预测值、标准差或适用域缺失会标记为 `unknown`，不会被当作通过。
6. 每个角色单独设置熔点上限；系统按 `预测熔点 + 标准差 <= 上限`、标准差上限和最低适用域分数共同判断。没有启用的角色不会被虚构出熔点结果。

熔点数据字段建议至少包含：`smiles`、`mp_c`、`mp_raw`、`mp_quality`、`component_role`、`cid`、`source_url` 和 `canonical_smiles`。PubChem 文本中的范围、分解、估算和混合物记录会被保留并分类，便于复核来源；训练时应优先使用高质量单值记录。
...
**熔点筛选:**
- 树脂和固化剂分别设置熔点上限，避免用同一个阈值掩盖不同角色的分布差异。
- 先使用 `annotate` 检查预测、标准差和适用域，再切换到 `strict` 做最终导出。
- 对低于 130°C 的目标，建议先放宽适用域阈值观察候选覆盖率，再逐步收紧，而不是直接扩大经验公式的范围。
- 熔点模型必须使用与训练时一致的特征工作流；若模型 artifact 缺少熔点任务标记、目标列或单位，系统应拒绝将其用于熔点门控。
