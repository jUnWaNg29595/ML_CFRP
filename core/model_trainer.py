...
def _apply_parallel_settings(model_name: str, model_params: dict, train_n_jobs: int):
    ...
    threads = (os.cpu_count() or 1) if train_n_jobs == -1 else max(1, int(train_n_jobs))

    # sklearn-style n_jobs models
    if model_name in {"线性回归", "Ridge回归", "随机森林", "Extra Trees", "XGBoost", "LightGBM"}:
        model_params.setdefault("n_jobs", train_n_jobs)
