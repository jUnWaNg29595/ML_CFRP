...
def _decimer_model_zip_candidates() -> list[str]:
    ...
    candidates.append(os.path.join(home, ".data", "DECIMER-V2", "DECIMER_Img2Mol_Benchmark", "models.zip"))
    candidates.append(os.path.join(home, ".cache", "DECIMER-V2", "DECIMER_Img2Mol_Benchmark", "models.zip"))
    candidates.append(os.path.join(home, "AppData", "Local", "DECIMER-V2", "DECIMER_Img2Mol_Benchmark", "models.zip"))
    return candidates


def _decimer_model_dirs() -> list[str]:
    """Possible DECIMER cache directories that may contain extracted or partial model files."""
    home = os.path.expanduser("~")
    return [
        os.path.join(home, ".data", "DECIMER-V2"),
        os.path.join(home, ".cache", "DECIMER-V2"),
        os.path.join(home, "DECIMER-V2"),
        os.path.join(home, "AppData", "Local", "DECIMER-V2"),
    ]
...
def _clear_decimer_cache_dir() -> None:
    """Best-effort cleanup of partially extracted DECIMER cache files after a broken download."""
    try:
        import shutil
    except Exception:
        return

    for base in _decimer_model_dirs():
        for name in ["DECIMER_Img2Mol_Benchmark", "DECIMER-V2", "models"]:
