...
class Visualizer:
    ...
    def plot_parity_train_test(self, y_train, y_pred_train, y_test, y_pred_test, target_name="Target"):
        ...
        ax.scatter(y_train, y_pred_train,
                   c='#87CEFA', label='Train', marker='o', s=25, alpha=0.8, edgecolors='none', zorder=2)

        # 3. 绘制测试集 (蓝色菱形)
        ax.scatter(y_test, y_pred_test,
                   c='#FF0000', label='Test', marker='d', s=30, alpha=0.9, edgecolors='none', zorder=3)
        ...
        label_name = target_name.split('_')[0] if '_' in target_name else target_name
        ...
        # 生成导出数据 (合并训练集和测试集，并打标)
        df_train = pd.DataFrame({"True_Value": y_train, "Predicted_Value": y_pred_train, "Set": "Train"})
        df_test = pd.DataFrame({"True_Value": y_test, "Predicted_Value": y_pred_test, "Set": "Test"})
        export_df = pd.concat([df_train, df_test], ignore_index=True)
